...
import com.apollo.demos.osgi.base.impl.Utilities.percent
...
class PoolSummary extends CommandBase {
  def onCommand() {
    ...
    val ps = tpPs + fjpPs
    println(table(List(("Threads", 2), ("Pool Threads", 3), ("Pool Rate", 2)), List(List(ts, ps, percent(ps, ts)))))

    val tpC = allThreadPool.size
    val tpAc = allThreadPool.map(_.getActiveCount).sum
    val tpQt = allThreadPool.map(_.getQueue.size).sum
    val tp = List("ThreadPool", tpC, tpPs, tpAc, percent(tpAc, tpPs), tpQt, percent(tpQt, tpPs))
    ...
    val fjp = List("ForkJoinPool", fjpC, fjpPs, fjpAc, percent(fjpAc, fjpPs), fjpQt, percent(fjpQt, fjpPs))
