...
import static com.apollo.demos.osgi.app.adapter.ptn.api.IPtnAdapterConstants.ComponentName.Processor5;
...
@Component
@Service
@Properties({ @Property(name = FunctionId, value = "80002"),
             @Property(name = DeviceType, value = "ZXCTN_6120"),
             @Property(name = Version, value = "V1.00/V2.20") })
public class Function2 extends PtnBaseFunction {

    @Reference(target = ProcessorPrefix + Processor4 + ")")
    @Processor(order = 2)
    protected volatile ComponentFactory processor4;

    @Reference(target = ProcessorPrefix + Processor5 + ")")
    @Processor(order = 3)
    protected volatile ComponentFactory processor5;
